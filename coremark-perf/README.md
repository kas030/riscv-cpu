...
日常迭代默认只仿真 2 次，并在同一次运行的第 1、2 次迭代抓取统计，外推
...
| 模式 | 实跑次数 | 观察点 | 用途 |
|---|---:|---:|---|
| `estimate` | 2 | 1 / 2 | 最快日常优化与 10000 次外推 |
| `quick` | 1 | 1 | 最短 CRC/路径检查，不生成外推报告 |
| `stage` | 16 | 8 / 16 | 阶段性检查线性与稳定性 |
...
每个 CoreMark 迭代调用该函数两次；第二次入口作为观察点。任意两个同类观察点的
差值严格覆盖整数个完整迭代，因此：

```text
每次增量 = snapshot2 - snapshot1
metric10000 = final2 + (10000 - 2) × 每次增量
```

`metric10000` 保留启动 RT-Thread、调度 autorun 线程、初始化和结果输出等固定开销。
报告还会用 `cycles/iteration` 单独计算稳态 10000 次时间与
`iterations/second`，避免 COUNTER 的 1 ms 量化误差。
