# RT-Thread 材料版 CoreMark 18000 次性能验证

本目录用于验证当前 CPU 在 RT-Thread Nano 上运行材料包 CoreMark
`2K performance` 负载 18000 次的时间。板上命令为：
...
18000 次结果。这样保留 RT-Thread 启动、CoreMark 初始化、完整算法、
CRC 校验和返回 shell 的真实路径，同时把开发仿真从约 23 秒目标运行缩短到约
0.04 秒仿真时间。

为进一步减少仿真固定开销，性能固件会启用 `COREMARK_PERF_AUTORUN`：调度器启动
后由一个 RT-Thread 线程直接调用材料的 `coremark_main(void)`。该开关默认关闭，
不改变正常 RT-Thread 固件；它只省去仿真 UART 单字节接收、Team ID 交互和 shell
文本解析，不省略 CoreMark 的初始化、计时、算法、CRC 或报告路径。
...
| 模式 | 实跑次数 | 观察点 | 用途 |
|---|---:|---:|---|
| `estimate` | 16 | 8 / 16 | 日常验证与 18000 次外推 |
| `quick` | 1 | 1 | 最短 CRC/路径检查，不生成外推报告 |
| `stage` | 64 | 32 / 64 | 更低计时量化误差的阶段检查 |
| `full` | 18000 | 9000 / 18000 | 正式有效运行和最终验收 |
...
metric18000 = final16 + (18000 - 16) × 每次增量
```
...
`full` 还必须执行 18000 次、运行至少 10 秒、输出
...
受保护输入由 [protected_sources.sha256](protected_sources.sha256) 固定为材料 ZIP。
每次构建前后都会校验，任何差异都会使脚本失败。构建生成的
`coremark-handout/provenance.json` 同时记录压缩包、原始成员和实际编译源哈希；
`core_list_join.c`、`core_matrix.c`、`core_state.c`、`core_util.c`、`core_main.c`
及 `coremark.h` 的两组哈希必须完全相同。材料平台文件只允许生成脚本中列明的
板级适配。
