...
IF 级还使用 256 项直接映射双发射提示表，记录某个取指包能否安全双发射。tag 未命中、冷启动或发生预测跳转时先按单发射执行，待双路 IROM 返回后再完成指令类型、包内 RAW 和资源冲突判断并训练表项。提示表命中后可直接选择 `PC+4` 或 `PC+8`，避免每拍把第二路 IROM 的完整译码串入下一 PC 反馈路径。由于 IROM 内容在运行期间保持不变，命中项可复用此前对同一取指包得到的保守判定；该结构主要优化循环和重复执行代码。
