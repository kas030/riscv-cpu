...
Windows 字体。本项目在 Debian 13 上构建设计报告时使用下列替代字体：
...
```sh
mkdir -p build
pandoc design-report.md \
  --defaults=/tmp/riscv-report-linux.yaml \
  --output=build/design-report.pdf
```

生成文件为 `docs/build/design-report.pdf`。`/tmp` 可能在重启后被清理，遇到临时
配置不存在时，重新执行上述命令即可。
