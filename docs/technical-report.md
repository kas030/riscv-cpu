...
| COUNTER | `0x8020_0050` | 毫秒计数，特定写值控制启动和停止 |
| COUNTER_US | `0x8020_0054` | 同一 50 MHz 计时器的微秒计数，只读 |
...
向 COUNTER 写入 `0x8000_0000` 后，毫秒和微秒读口同时开始计时；写入 `0xFFFF_FFFF` 后同时停止。I2C 主机工作在 200 MHz CPU 域，产生 100 kHz 开漏 SCL/SDA，并通过 J7 连接 BME280。UART 在 200 MHz CPU 域和 50 MHz 外设域之间完成握手。CPU 总线没有 ready/valid、重试或错误响应，任何接口修改都必须保持现有返回拍数。
