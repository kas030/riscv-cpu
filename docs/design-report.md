...
取指端可同时读取 `pc` 和 `pc+4` 对应的两条指令。若包内不存在 RAW 相关或资源冲突，两条指令分别进入两个执行槽，PC 前进 8 字节；否则只发射第一槽，PC 前进 4 字节。遇到控制流、CSR、双访存或双 RV32M 组合时，处理器改为单发射。两槽仍按程序顺序提交，第一槽先于第二槽。
...
取指端设有一张直接映射的双发射提示表。同步 IROM 返回两条指令后，硬件判断它们能否配对，并据此训练对应表项；冷启动或 tag 未命中时先按单发射处理。条件分支由 64 项 2 位饱和计数 BHT 预测，尚未训练的分支使用 BTFNT。`jal` 在 IF 级预测跳转，`jalr`、`ecall` 和 `mret` 留到 EX 级解析。
...
CPU 对外只有一组数据访问接口，因此每拍最多发射一条 load/store。MEM1 从两个槽中选出有效的访存指令，输出地址、写数据和字节写掩码；MEM2 接收同步 BRAM 返回的完整 32 位字；WB 再根据地址低位和 `funct3` 选择 byte/halfword，并完成符号扩展。

LSU 按小端字节序生成 `sb/sh/sw` 所需的写数据和 byte mask。数据 BRAM 的地址范围是 `0x80100000` 至 `0x8013FFFF`，容量为 256 KiB。SW、KEY、SEG、LED 和 COUNTER 映射在 `0x80200000` 附近，具体地址以 `perip_bridge.sv` 的译码为准。
...
ID 级还会检查包内 RAW/WAW 相关和资源冲突。控制流、CSR、双访存及双 RV32M 组合均按单发射执行。EX 级把实际分支结果与预测元数据进行比较；方向或目标不一致时重定向 PC，并冲刷 IF/ID 和 ID/EX，预测正确时则让流水线继续执行。
...
| FPGA 板级综合程序 | 约 404,056,765 | 同一程序 | 约 1.062 | COUNTER 计时 1,683 ms |

Table: CPU 性能结果摘要

表中的板级执行时间按 240 MHz CPU 时钟计算，计时器结果与仿真周期换算基本一致。仓库中的 PLL、BRAM IP 和仿真配置目前都使用 240 MHz CPU 时钟。由于整理本报告时没有重新运行 Vivado 综合及布局布线，240 MHz 只能代表当前工程配置，不能视为处理器的极限频率。最终时序应以对应实现报告中的 WNS 和 route status 为准。
