...
| 7 | 右上 | `0x1000_0000` | `0x01c8` | 固定置位；dump 中未发现独立测试返回值 | 6 |
| 8 | 左上 | `0x2000_0000` | `0x01dc` | 固定置位；dump 中未发现独立测试返回值 | 6 |
...
- 图片编号和 LED mask 的位置对应是自洽的：`0x1/0x2` 对应底行 1/2，`0x100/0x200` 对应上一行 3/4，`0x10000/0x20000` 对应 5/6，`0x10000000/0x20000000` 对应顶行 7/8。
