...
| `0x80200078` | SEG_RAW_LO | 是 | 是 | 右侧四位原始段码；每字节 bit7=小数点、bit6:0=段码 |
| `0x8020007C` | SEG_RAW_HI | 是 | 是 | 左侧四位原始段码；写入后启用原始段码显示模式 |

COUNTER 写入 `0x80000000` 开始计数，写入 `0xFFFFFFFF` 停止计数。

写普通 SEG 寄存器会退出原始段码模式并恢复旧的 8 位十六进制显示，因而旧软件无需修改。
