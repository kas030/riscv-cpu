...
const reg = (index) => ABI[index] ?? `x${index}`;
...
export function disassemble(hex) {
  ...
  const iImm = signed(instruction >>> 20, 12);
  const sImm = signed(((instruction >>> 25) << 5) | ((instruction >>> 7) & 0x1f), 12);
  const bImm = signed((((instruction >>> 31) & 1) << 12) | (((instruction >>> 7) & 1) << 11) | (((instruction >>> 25) & 0x3f) << 5) | (((instruction >>> 8) & 0xf) << 1), 13);
  const jImm = signed((((instruction >>> 31) & 1) << 20) | (((instruction >>> 12) & 0xff) << 12) | (((instruction >>> 20) & 1) << 11) | (((instruction >>> 21) & 0x3ff) << 1), 21);
  const uImm = instruction & 0xfffff000;
  if (instruction === 0x00000073) return "ecall";
  if (instruction === 0x30200073) return "mret";
  if (opcode === 0x37) return `lui ${reg(rd)},0x${(uImm >>> 12).toString(16)}`;
  if (opcode === 0x17) return `auipc ${reg(rd)},0x${(uImm >>> 12).toString(16)}`;
  if (opcode === 0x6f) return `jal ${reg(rd)},${jImm}`;
  if (opcode === 0x67 && funct3 === 0) return `jalr ${reg(rd)},${iImm}(${reg(rs1)})`;
  if (opcode === 0x63) return `${["beq", "bne", "?", "?", "blt", "bge", "bltu", "bgeu"][funct3]} ${reg(rs1)},${reg(rs2)},${bImm}`;
  if (opcode === 0x03) return `${["lb", "lh", "lw", "?", "lbu", "lhu", "?"][funct3]} ${reg(rd)},${iImm}(${reg(rs1)})`;
  if (opcode === 0x23) return `${["sb", "sh", "sw"][funct3] ?? "?"} ${reg(rs2)},${sImm}(${reg(rs1)})`;
  if (opcode === 0x13) {
    if (funct3 === 1) return `slli ${reg(rd)},${reg(rs1)},${rs2}`;
    if (funct3 === 5) return `${funct7 === 0x20 ? "srai" : "srli"} ${reg(rd)},${reg(rs1)},${rs2}`;
    return `${["addi", "slli", "slti", "sltiu", "xori", "?", "ori", "andi"][funct3]} ${reg(rd)},${reg(rs1)},${iImm}`;
  }
  if (opcode === 0x33) {
