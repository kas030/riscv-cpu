...
static void shell_auto_complete(char *prefix)
{
    ...
    {
#ifdef FINSH_USING_MSH_AUTO_COMPLETE
...
void finsh_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        /* 本工具链默认 -funsigned-char：无输入时 rt_hw_console_getchar
         * 返回 0xFF 而非 -1（上方的 ch < 0 分支是死代码）。此处必须让出
         * CPU，否则 shell 忙等会饿死 idle 线程（mycpu 的 tick 由 idle
         * hook 轮询 COUNTER 产生），系统时间不再前进。1ms 轮询不会丢
         * 9600 波特（1.04ms/字节）的输入：bridge 单字节锁存保持到被读。 */
        if (ch == '\0' || ch == 0xFF) { rt_thread_mdelay(1); continue; }
        /* handle tab key */
        else if (ch == '\t')
        {
            int i;
