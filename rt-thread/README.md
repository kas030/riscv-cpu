...
裸命令会先要求输入 Team ID，随后使用标准性能种子 `0,0,0x66`、总数据量
2000 字节和 18000 次迭代。性能脚本会显式覆盖次数以保持其 10000 次实验口径。
200 MHz 配置下，CoreMark 对象使用 `-O3`、其余固件保持 `-Os`；输出应包含
`Correct operation validated`、三项 CRC `e714 / 1fd7 / 8e3a` 和 CoreMark 分数。
官方验证种子可另行运行：

```text
msh >coremark 0x3415 0x3415 0x66 18000
```
...
熄灭时停止秒表。裸命令由 msh 包装层补齐为标准性能种子和 18000 次；性能脚本
显式传入自身的迭代次数。仍可通过第 4 个参数指定其他次数；只有明确把第 4 个
参数设为 0 时，官方自动校准才会产生多次 LED 脉冲。
