...
//   每一位对应 alu 的一种运算（RV32I 算术逻辑/比较、RV32M 及 zext.h）。
...
module alu_ctrl(
    ...
    localparam logic [`ALU_OP_WIDTH - 1:0] OP_ZEXTH  = 23'h400000;
    ...
    assign ALUControl = {`ALU_OP_WIDTH{do_add   }} & OP_ADD    |
                        ...
                        {`ALU_OP_WIDTH{do_zexth }} & OP_ZEXTH;
    ...
    // 完整性能镜像的 CRC 内核由工具链生成这一条 zext.h；其余抽选 Z
    // 指令仍保持未实现，并由测试结果小灯 2 单独反映。
    assign do_zexth  = kind_r && funct7 == 7'b0000100 && funct3 == 3'b100;
endmodule
