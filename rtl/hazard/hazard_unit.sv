...
//   集中检测两类冒险并输出停顿/冲刷信号：
//     1) Load-Use 冒险：ID/EX load 若提前探测 L0 命中则可在下一拍从
//        MEM1 前递，否则停顿；EX/MEM(MEM1) miss 继续等待 WB。
...
module hazard_unit (
    ...
    output logic       LoadUseEX     ,
    output logic       LoadUseMEM
);
    // load-use 检测：提前命中的 EX load 可在下一拍由 MEM1 直接前递。
    ...
    assign load_use_ex  = (ID_EX_MemRead && !ID_EX_LoadReady &&
                           (ID_EX_rd != 5'd0) &&
                           (hit_id_ex_slot0 || hit_id_ex_slot1)) ||
                          (ID_EX_MemRead_1 && !ID_EX_LoadReady_1 &&
                           (ID_EX_rd_1 != 5'd0) &&
    ...
    assign LoadUseEX    = load_use_ex;
    assign LoadUseMEM   = load_use_mem;
endmodule
