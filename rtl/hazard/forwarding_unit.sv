...
//     2'b11  从 MEM2 流水寄存器前递
//     2'b01  从 MEM/WB 流水寄存器前递（再前一拍）
//     2'b00  无前递，使用寄存器堆原始读出值
//   优先级：EX/MEM > MEM2 > MEM/WB > 寄存器堆，因为越近的指令值越新。
...
module forwarding_unit (
    ...
    input  logic [31:0] MEM2_rd_oh     ,                    // MEM2 写回目标 one-hot
    ...
    // A 端前递：EX/MEM 优先，其次 MEM2、MEM/WB
    always_comb begin
        if      (EX_MEM_rd_oh[ID_EX_rs1])
            ForwardA = 2'b10;
        else if (MEM2_rd_oh[ID_EX_rs1])
            ForwardA = 2'b11;
