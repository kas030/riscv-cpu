...
//     [8] beq  [9] bne [10] blt [11] bge [12] bgeu [13] bltu [22] clz
//   设计要点：
...
module alu #(
    ...
    logic m_sra , m_beq , m_bne , m_blt , m_bge , m_bgeu, m_bltu, m_clz, m_crc8;
    logic [DATAWIDTH-1:0] add_lhs, add_rhs;
    logic                 cin, cout;
    logic [DATAWIDTH-1:0] r_addsub, r_and, r_or, r_xor;
    logic [DATAWIDTH-1:0] r_sll, r_srl, r_sra, r_clz, r_crc8;
    ...
    always_comb begin
        r_clz = DATAWIDTH;
        clz_found = 1'b0;
        for (clz_i = DATAWIDTH - 1; clz_i >= 0; clz_i = clz_i - 1) begin
            if (!clz_found && A[clz_i]) begin
                r_clz = DATAWIDTH - 1 - clz_i;
                clz_found = 1'b1;
            end
        end
    end
