...
module alu #(
    ...
    logic [DATAWIDTH-1:0] r_clmulh;
    logic                 cmp_eq, cmp_lt, cmp_ltu;
    logic [2*DATAWIDTH-1:0] cl_product;
    integer                 cl_i;
    ...
    always_comb begin
        cl_product = '0;
        for (cl_i = 0; cl_i < DATAWIDTH; cl_i = cl_i + 1)
            if (B[cl_i])
                cl_product = cl_product ^ ({{DATAWIDTH{1'b0}}, A} << cl_i);
    end
