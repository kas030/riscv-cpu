...
//   - 维护 PC 寄存器，根据 BranchTaken 在 (PC+4) 与 IF_npc_redirect 间二选一
//   - 把 PC 输出到 IROM 的地址端口，IROM 同周期返回的 32 位指令直接连给
//     IF_instr，再交给 IF/ID 流水寄存器锁存。
//   - Stall=1 时由 PC 模块的 en 入口冻结 PC，使流水线整体停顿。
// =============================================================================
module myCPU_if_stage #(
    ...
    // 把 PC 当作取指地址送给 IROM；IROM 当周期吐出的数据即为指令
