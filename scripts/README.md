...
- `create_project.tcl`
  ...
  - 导入 RTL、约束、COE/MIF 文件，以及 `ip/IROM`、`ip/DRAM`、`ip/pll`
    下已有的 XCI IP 文件。
  - 创建后会对所有 XCI 执行 `generate_target all`。

- `recreate_dram_bram_ip.tcl`
  - 专门用于把 Vivado 工程内的 `DRAM` IP 重建为 `blk_mem_gen`
    True Dual Port BRAM。
  - 适用于综合报旧 `DRAM_stub.v` 端口不匹配，例如 RTL 连接了
...
这里的 `DRAM` 是工程中“数据存储器”的逻辑名称，不代表底层一定使用
distributed RAM 或外部 DRAM。将实现改为 BRAM 后仍保留模块名 `DRAM`，是为了
保持以下内容稳定：

- RTL 中 `DRAM Mem_DRAM (...)` 的模块名和实例名。
- Vivado 工程内已有 IP 名称。
- testbench、波形配置、调试脚本中可能存在的层次路径。
- 文档和地址映射中对“数据存储器区域”的称呼。

如果把 IP 改名为 `BRAM`，需要同步修改 RTL 实例、XCI、Vivado 工程文件、仿真
配置和层次引用，收益有限且容易引入无关错误。
...
```text
[Synth 8-11365] ... module 'DRAM' ... named port connection 'clka' does not exist
...
```text
vivado/digital_twin.gen/sources_1/ip/DRAM/DRAM_stub.v
vivado/digital_twin.runs/synth_1/.Xil/.../DRAM_stub.v
```
...
- `sim/coe/dram.coe` 存在，用作 BRAM 初始化文件。
...
```text
INFO: recreated DRAM as blk_mem_gen true dual-port BRAM: .../DRAM.xci
```
...
`recreate_dram_bram_ip.tcl` 会：

1. 打开 `vivado/digital_twin.xpr`。
2. 从工程中移除旧 `DRAM` IP。
3. 删除工程源目录下旧的 `DRAM.xci`。
4. 通过 `create_ip -name blk_mem_gen ... -module_name DRAM` 重新创建同名 IP。
